...
int held_modifier = 0;
HKL test_layout = reinterpret_cast<HKL>(0x0409);
HWND test_ime_window = nullptr;
DWORD test_conversion_mode = 0;
HKL WINAPI test_keyboard_layout(DWORD) { return test_layout; }
HWND WINAPI test_default_ime_window(HWND) { return test_ime_window; }
LRESULT CALLBACK test_ime_proc(HWND window, UINT message, WPARAM command, LPARAM parameter) {
    if (message == WM_IME_CONTROL) {
        if (command == 0x0001) { return test_conversion_mode; }
    }
    return DefWindowProcW(window, message, command, parameter);
}
...
#define GetKeyboardLayout test_keyboard_layout
#define ImmGetDefaultIMEWnd test_default_ime_window
...
void korean_hangul_must_not_emit_accent() {
    WNDCLASSW ime_class{};
    ime_class.lpfnWndProc = test_ime_proc;
    ime_class.lpszClassName = L"FAI.TestIME";
    expect(RegisterClassW(&ime_class) != 0, "test IME window class must register");
    test_ime_window = CreateWindowExW(0, ime_class.lpszClassName, L"", 0,
                                      0, 0, 0, 0, nullptr, nullptr, nullptr, nullptr);
    test_focus = CreateWindowExW(0, L"STATIC", L"", 0,
                                 0, 0, 0, 0, nullptr, nullptr, nullptr, nullptr);
    expect(test_ime_window && test_focus, "hidden test windows must exist");
    if (test_ime_window && test_focus) {
        KBDLLHOOKSTRUCT event{};
        event.vkCode = 'E';
        event.flags = LLKHF_ALTDOWN;
        test_layout = reinterpret_cast<HKL>(0x0412);
        for (const DWORD conversion : {DWORD{IME_CMODE_NATIVE}, DWORD{0}}) {
            router = fai::InputRouter{};
            (void)router.handle({VK_LMENU, true, false, 1});
            test_conversion_mode = conversion;
            captured_inputs.clear();
            (void)keyboard_proc(HC_ACTION, WM_SYSKEYDOWN, reinterpret_cast<LPARAM>(&event));
            expect(captured_inputs.empty() == (conversion == IME_CMODE_NATIVE),
                   "Hangul mode must pass E through; Korean IME A mode must accent it");
            const auto release = router.handle({'E', false, false, 1});
            expect(release.suppress == (conversion == 0),
                   "Hangul E release must pass; Korean IME A release must be consumed");
        }
        const HWND ime_window = test_ime_window;
        test_ime_window = nullptr;
        router = fai::InputRouter{};
        (void)router.handle({VK_LMENU, true, false, 1});
        captured_inputs.clear();
        (void)keyboard_proc(HC_ACTION, WM_SYSKEYDOWN, reinterpret_cast<LPARAM>(&event));
        expect(captured_inputs.empty(), "unreadable Korean IME must not inject Latin text");
        test_ime_window = ime_window;
        test_layout = reinterpret_cast<HKL>(0x0409);
        router = fai::InputRouter{};
        (void)router.handle({VK_LMENU, true, false, 1});
        test_conversion_mode = IME_CMODE_NATIVE;
        captured_inputs.clear();
        (void)keyboard_proc(HC_ACTION, WM_SYSKEYDOWN, reinterpret_cast<LPARAM>(&event));
        expect(!captured_inputs.empty(), "English layout must keep accent behavior");
    }
    if (test_focus) { DestroyWindow(test_focus); }
    if (test_ime_window) { DestroyWindow(test_ime_window); }
    test_focus = nullptr;
    test_ime_window = nullptr;
    test_layout = reinterpret_cast<HKL>(0x0409);
}
