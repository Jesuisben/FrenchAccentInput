...
1. `src/main.cpp`는 VK 0xE8 mask와 실제 VK_BACK으로 수정됐다. 두 변경 각각 output test RED/GREEN과 메모장 synthetic GUI 차이를 확인했다. Ctrl-mask/Unicode Backspace로 되돌리지 않는다.
   - 이전 generic 경로는 메모장 fast-mapping에서 실패했다. 현재 native EDIT/RichEdit에는 EM_GETSEL/EM_SETSEL/EM_REPLACESEL 경로가 있으며 긴 문서·readonly·text limit·caret 변경 검사를 포함한다. 이를 제거하거나 Alt 복원만 생략하는 변경으로 되돌리지 않는다. 최신 source의 GUI 재검증은 별도로 수행한다.
...
3. 메모장 clean field, browser, VS Code에서 mapping/cycle/일반 입력/UI 복귀를 추가 검증한다. 외부 injected keyboard·mouse의 sequence 취소와 부분 SendInput 복구는 RED/GREEN 및 CTest 2/2로 확인했다. 복구도 차단될 수 있으므로 성공으로 승격하지 않는다. modifier 초기 상태, focus/caret 변경, shortcut 비간섭의 남은 source/test 누락을 점검한다.
